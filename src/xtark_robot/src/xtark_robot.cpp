...
// {
//     //ROS初始化 并设置节点名称 
//     ros::init(argc,argv,"xtark_robot_node");

//     ROS_INFO("Xtark Robot is Starting ......  ");
...
//     RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"Xtark Robot is Starting ......  ");
    
//     //实例化一个对象
//     XtarkRobot robot;
//     rclcpp::spin(std::make_shared<XtarkRobot>());
//     rclcpp::shutdown();
...
//     //仅AKM 机器人舵机零偏参数初始化
//     private_nh_.param<int>        ("akm_servo_offset", akm_servo_offset_, 0);
//     //模式选择
...
//     //实例化订阅者对象
//     cmd_vel_sub_ = nh_.subscribe<geometry_msgs::Twist>(cmd_vel_topic_, 100, &XtarkRobot::cmdVelCallback, this);
//     light_sub_ = nh_.subscribe<std_msgs::ColorRGBA>("light",10,&XtarkRobot::light_callback,this);
//     //数据初始化
//     memset(&pos_data_, 0, sizeof(pos_data_));

//     //提示信息，串口端口号和波特率
//     ROS_INFO("Xtark Robot Set serial %s at %d baud", serial_port_.c_str(), serial_port_baud_);
...
//         catch(...)
//         {
//             ROS_INFO("Xtark Robot can not open recvSerial_thread, Please check the serial port cable ");
...
//     ROS_INFO("Xtark Robot is Connected to OpenCRP board ");
...
//     ROS_INFO("Xtark Robot Set AKM Servo Offset Angle %d, other ignore ", akm_servo_offset_);

//     //动态参数配置  
//     dynamic_reconfigure::Server<xtark_robot::robotConfig> reconfig_server;
// 	dynamic_reconfigure::Server<xtark_robot::robotConfig>::CallbackType f;
// 	f = boost::bind(&XtarkRobot::dynamicReconfigCallback,this,_1,_2);
...
//     ROS_INFO("Xtark Robot Set color_mode:%d, color_R:%d, color_G:%d, color_B:%d", RGB_M1_,RGB_R_,RGB_G_,RGB_B_);

//     //提示信息，发布订阅的话题消息
//     ROS_INFO("Xtark Robot setup publisher  on odom [nav_msgs/Odometry]");
//     ROS_INFO("Xtark Robot setup publisher  on imu [sensor_msgs/Imu]");
//     ROS_INFO("Xtark Robot setup publisher  on bat_vol [float]");
//     ROS_INFO("Xtark Robot setup subscriber on cmd_vel [geometry_msgs/Twist]");

//     //机器人启动完成提示信息
//     ROS_INFO("Xtark Robot initialization completed, is Running!");
...
//ROS2
XtarkRobot::XtarkRobot(const rclcpp::NodeOptions & options):Node("xtark_robot_node", options)
,tf_broadcaster_(this)
{   
    //创建节点句柄
    // nh_ = rclcpp::Node::make_shared("xtark_robot_node");
    ...
    "/cmd_vel", 100,
    std::bind(&XtarkRobot::cmdVelCallback, this, std::placeholders::_1));

    light_sub_ = this->create_subscription<std_msgs::msg::ColorRGBA>(
    "/light", 10,
    std::bind(&XtarkRobot::light_callback, this, std::placeholders::_1));
    ...
    // 提示信息，串口端口号和波特率
    RCLCPP_INFO(this->get_logger(), "Xtark Robot Set serial %s at %d baud", serial_port_.c_str(), serial_port_baud_);
    ...
    offset_data[1] = (int16_t)(akm_servo_offset_);
    sendSerialPacket(offset_data, 2, ID_ROS2CRP_AKM);
    RCLCPP_INFO(this->get_logger(), "Xtark Robot Set AKM Servo Offset Angle %d, other ignore ", akm_servo_offset_);

    // 动态参数配置  
    // ROS2中的动态参数配置稍有不同，需要使用Parameter Server
    ...
    init_color[5] = RGB_B_;
    sendSerialPacket(init_color, 6, ID_ROS2CRP_LGT);
    RCLCPP_INFO(this->get_logger(), "Xtark Robot Set color_mode:%d, color_R:%d, color_G:%d, color_B:%d", RGB_M1_,RGB_R_,RGB_G_,RGB_B_);
...
{ 
    ...
    //提示信息
    //ROS1
    // ROS_INFO("Xtark Robot shutting down."); 
...
void XtarkRobot::light_callback(const std::shared_ptr<const std_msgs::msg::ColorRGBA>& light)
{
...
//ROS1
// void XtarkRobot::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg)
// {
//     static uint8_t vel_data[11];
...
void XtarkRobot::SetColor(float r, float g, float b, float a , uint8_t selet)
{
    ...
    //发送串口数据
    sendSerialPacket(send_buff, 6, ID_ROS2CRP_LGT);
    RCLCPP_INFO(this->get_logger(), "Light Control : selet: %d R: %d, G: %d, B: %d",selet,send_buff[6],send_buff[7],send_buff[8]);
}
...
//ROS1
// void XtarkRobot::dynamicReconfigCallback(xtark_robot::robotConfig &config,uint32_t level)
// {       
//     uint8_t data[2];  
...
//         data[0] = 0x55;
//         data[1] = 0x55;
//         sendSerialPacket(data, 2, ID_ROS2CRP_IMU);   
...
//     //阿克曼发送零位偏执修正角度更新
//     if(config.akm_servo_offset != akm_servo_offset_)
//     {
//         akm_servo_offset_ = config.akm_servo_offset;
//         ROS_INFO("Set akm servo Offset:[%d], Other  robot ignore. ", akm_servo_offset_);

//         //发送舵机零偏修正角
//         data[0] = (int16_t)(akm_servo_offset_)>>8;
//         data[1] = (int16_t)(akm_servo_offset_);
//         sendSerialPacket(data, 2, ID_ROS2CRP_AKM);          
//     }
...
//         sendSerialPacket(data_light, 6, ID_ROS2CRP_LGT);
//         ROS_INFO("Xtark Robot Set color_mode_reconfig:%d, color_R:%d, color_G:%d, color_B:%d", config.light_chose,config.RGB_R,config.RGB_G,config.RGB_B);         
//     }
// }

//ROS2
// void XtarkRobot::dynamicReconfigCallback(xtark_robot::msg::RobotConfig::SharedPtr config)
// {       
//     uint8_t data[2];  
//     uint8_t data_light[6];
//     //IMU校准指令
...
//     {
//         akm_servo_offset_ = config->akm_servo_offset;
//         RCLCPP_INFO(this->get_logger(), "Set akm servo Offset:[%d], Other  robot ignore. ", akm_servo_offset_);
...
//         data_light[5] = (uint8_t)(config->RGB_B);
//         sendSerialPacket(data_light, 6, ID_ROS2CRP_LGT);
//         RCLCPP_INFO(this->get_logger(), "Xtark Robot Set color_mode_reconfig:%d, color_R:%d, color_G:%d, color_B:%d", config->light_chose,config->RGB_R,config->RGB_G,config->RGB_B);
//     }
// }
 ...
 * @功  能  计算IMU四元数
 */
void XtarkRobot::calculateImuQuaternion(struct imu_data imu_cel)
{
    float recipNorm;
